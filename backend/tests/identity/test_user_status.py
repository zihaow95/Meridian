...
def test_disable_user_records_disabled_at(active_user) -> None:
    ChangeUserStatus(target=active_user, status=UserStatus.DISABLED).execute()
...
def test_depart_user_records_departed_at(active_user) -> None:
    ChangeUserStatus(target=active_user, status=UserStatus.DEPARTED).execute()
    active_user.refresh_from_db()
...
def test_activate_user_records_activated_at(active_user) -> None:
    ...
    ChangeUserStatus(target=active_user, status=UserStatus.ACTIVE).execute()
    active_user.refresh_from_db()
...
def test_status_change_does_not_delete_identity_binding(active_user) -> None:
    ...
    ChangeUserStatus(target=active_user, status=UserStatus.DISABLED, at=timezone.now()).execute()
