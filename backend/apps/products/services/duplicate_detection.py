...
class DetectProductImportDuplicates:
    ...
    def execute(self) -> list[DuplicateCandidate]:
        return detect_duplicate_candidates(
            ...
            payload=self.item.normalized_payload,
        )


def detect_duplicate_candidates(
    *, organization_id: int, payload: dict[str, Any]
) -> list[DuplicateCandidate]:
    """Find products that may already be the one being entered.

    Shared by the spreadsheet importer and the item-by-item form so both offer
    the same candidates; neither ever merges anything on its own.
    """
