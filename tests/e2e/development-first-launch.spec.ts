...
async function createProjectFromOpportunity(
  ...
  const projectsResponse = await page.request.get('/api/v1/projects?page=1&page_size=100')
  expect(projectsResponse.ok()).toBeTruthy()
  const projects = await projectsResponse.json()
  const created = projects.items.find((item: { name: string }) => item.name === uniqueTitle)
...
async function findProjectByBusinessNo(
  ...
  const response = await page.request.get('/api/v1/projects?page=1&page_size=100')
  expect(response.ok()).toBeTruthy()
  const payload = await response.json()
  const item = payload.items.find((row: { business_no: string }) => row.business_no === businessNo)
...
async function stageGateIdFor(
  ...
  const stages = await (await page.request.get(`/api/v1/projects/${projectPublicId}/stages`)).json()
  const stage = stages.items.find((item: { stage_code: string }) => item.stage_code === stageCode)
  expect(stage?.stage_gate_public_id).toBeTruthy()
  return stage.stage_gate_public_id as string
}

test('new product creates runtime stages and opens project workbench', async ({ page }) => {
  ...
  const stagesResponse = await page.request.get(`/api/v1/projects/${projectPublicId}/stages`)
  expect(stagesResponse.ok()).toBeTruthy()
  const stages = await stagesResponse.json()
  const codes = stages.items.map((item: { stage_code: string }) => item.stage_code)
  expect(codes).toEqual(['D1', 'D2', 'D3', 'D4', 'D5', 'L1', 'L2', 'L3'])
  const l2 = stages.items.find((item: { stage_code: string }) => item.stage_code === 'L2')
...
test('incomplete gate submission is blocked until core task completes', async ({ page }) => {
  ...
  const validate = await authedJson(page, 'POST', `/api/v1/stage-gates/${gateId}/validate`)
  expect(validate.ok()).toBeTruthy()
  const body = await validate.json()
  expect(body.blocks.some((b: { code: string }) => b.code === 'CORE_TASK_INCOMPLETE')).toBeTruthy()
...
test('first launch publishes product and hands over monitoring', async ({ page }) => {
  test.setTimeout(120_000)
  await devLogin(page, '/projects')
  const project = await findProjectByBusinessNo(page, E2E_LAUNCH_BUSINESS_NO)
...
test('publish pending repair is recorded when launch publish fails', async ({ page }) => {
  test.setTimeout(120_000)
  await devLogin(page, '/projects')
  const project = await findProjectByBusinessNo(page, E2E_REPAIR_BUSINESS_NO)
  if (project.status === 'PUBLISH_PENDING_REPAIR') {
    await page.goto(`/projects/${project.public_id}`)
    await expect(page.locator('.el-alert__title')).toContainText('PUBLISH_PENDING_REPAIR')
    return
  }
...
test('migration continue from D3 skips prior stages; archive creates no project', async ({
  ...
  const continueBatch = await authedJson(page, 'POST', '/api/v1/project-migration-batches', {
    ...
    rows: [
      {
        ...
        history_tasks: [{ task_code: 'D1-LEGACY', name: 'Legacy D1', stage_code: 'D1' }],
        history_files: [],
      },
    ],
  })
  ...
  const archiveBatch = await authedJson(page, 'POST', '/api/v1/project-migration-batches', {
  ...
  const confirmArchive = await authedJson(
    ...
    { disposition: 'ARCHIVE_ONLY', idempotency_key: `confirm-archive-${stamp}` },
  )
...
test('limited user cannot confirm migration or create emergency execution', async ({ page }) => {
  test.setTimeout(120_000)
  await devLogin(page, '/todos', E2E_LIMITED_LOGIN_KEY)

  const denyMigrate = await authedJson(page, 'POST', '/api/v1/project-migration-batches', {
