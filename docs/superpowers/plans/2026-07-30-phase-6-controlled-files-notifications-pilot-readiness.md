...
- [x] 深链目标无权读统一返回404并显示“无权访问或内容不存在”，不泄露对象存在性。
...
- [x] 冷启动迁移后执行两次阶段6种子，比较产品、版本、SKU、渠道、配置快照、文件链、材料确认、通知、Todo、试用批次和反馈的行级稳定快照。
- [x] E2E逐一创建存量产品，关联当前/历史/待整理材料，专业确认并发布老品基线；验证重试不重复创建。
- [x] E2E覆盖六类通知、三级等级、未读/已读、Todo处理同步、关闭、深链实时404（存在性不披露）和摘要最小披露。
- [x] E2E覆盖临时账号成功、错误密码、停用用户、独立账号审计和非生产标识。
